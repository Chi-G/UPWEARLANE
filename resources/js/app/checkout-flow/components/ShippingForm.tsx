...
import { ShippingFormProps, ShippingAddress } from '@/types';
...
export default function ShippingForm({ onShippingComplete, initialData }: ShippingFormProps) {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    Object.keys(formData)?.forEach((key) => {
      const error = validateField(key as keyof ShippingAddress, formData?.[key as keyof ShippingAddress]);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="space-y-4">
        ...
        <div>
          ...
          <input
            ...
            className={`w-full h-12 px-4 bg-input border rounded-lg text-foreground placeholder:text-muted-foreground focus-ring transition-smooth ${
              errors?.fullName && touched?.fullName ? 'border-error' : 'border-border'
            }`}
        ...
        <div>
          ...
          <input
            ...
            onBlur={handleBlur}
            className={`w-full h-12 px-4 bg-input border rounded-lg text-foreground placeholder:text-muted-foreground focus-ring transition-smooth ${
              errors?.address && touched?.address ? 'border-error' : 'border-border'
            }`}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            ...
            <input
              ...
              onBlur={handleBlur}
              className={`w-full h-12 px-4 bg-input border rounded-lg text-foreground placeholder:text-muted-foreground focus-ring transition-smooth ${
                errors?.city && touched?.city ? 'border-error' : 'border-border'
              }`}
              placeholder="Lagos"
            />
          ...
          <div>
            ...
            <input
              ...
              onBlur={handleBlur}
              className={`w-full h-12 px-4 bg-input border rounded-lg text-foreground placeholder:text-muted-foreground focus-ring transition-smooth ${
                errors?.state && touched?.state ? 'border-error' : 'border-border'
              }`}
          ...
          <div>
            ...
            <input
              ...
              onBlur={handleBlur}
              className={`w-full h-12 px-4 bg-input border rounded-lg text-foreground placeholder:text-muted-foreground focus-ring transition-smooth ${
                errors?.postalCode && touched?.postalCode ? 'border-error' : 'border-border'
              }`}
              placeholder="100001"
            />
        ...
        <div>
          ...
          <div className="relative">
            <select
              ...
              onChange={handleChange}
              className="w-full h-12 pl-4 pr-10 bg-input border border-border rounded-lg text-foreground focus-ring transition-smooth appearance-none"
            >
      ...
      <button
        type="submit"
        className="w-full h-12 px-8 bg-primary hover:bg-primary/90 text-primary-foreground font-medium rounded-lg shadow-gold transition-smooth press-effect"
      >
